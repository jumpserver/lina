/**
 * Auto-generated from QA Mode B draft MS-100061
 * Source title: 用户组列表-标签搜索：测试同key和不同key标签搜索
...
const MS_ID = 'MS-100061'
const TITLE = `用户组列表-标签搜索：测试同key和不同key标签搜索`
const STEPS = [
  {
...
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户组；验证: 用户组列表-标签搜索：测试同key和不同key标签搜索`
