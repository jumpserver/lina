/**
 * Auto-generated from QA Mode B draft MS-104248
 * Source title: 用户管理-用户列表：创建用户，设置用户组
...
const MS_ID = 'MS-104248'
const TITLE = `用户管理-用户列表：创建用户，设置用户组`
const STEPS = [
  {
    "action": "在创建用户表单上，填写名称、用户名、邮箱，并设置用户组，其他按照要求填写。",
    "id": "step-1",
    "sideEffect": "reversible"
  }
]
const OUTCOMES = [
  "[1] 创建用户成功。返回用户列表，可查看用户组字段，显示正确。也可到用户组页面，查看该用户组详情页，可查看该用户组下有该用户。当授权规则有该用户组时，则用户拥有这个授权的资产权限。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：创建用户，设置用户组`
