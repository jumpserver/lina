/**
 * Auto-generated from QA Mode B draft MS-100602
 * Source title: 克隆用户组表单优化为抽屉式表单（右侧弹出）
...
const MS_ID = 'MS-100602'
const TITLE = `克隆用户组表单优化为抽屉式表单（右侧弹出）`
const STEPS = [
  {
    "action": "用户管理-用户组：点击克隆用户组",
    "id": "step-1",
    "sideEffect": "reversible"
  },
  {
    "action": "表单填写，修改名称，修改用户，点击更新",
    "id": "step-2",
    "sideEffect": "reversible"
  }
]
const OUTCOMES = [
  "[1] 右侧弹出抽屉式表单，表单克隆了该用户组的信息",
  "[2] 更新成功过。显示用户组列表，查看该用户组信息是更新后信息正确的。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户组；验证: 克隆用户组表单优化为抽屉式表单（右侧弹出）`
