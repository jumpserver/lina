/**
 * Auto-generated from QA Mode B draft MS-104214
 * Source title: 用户管理-用户列表：使用系统管理员创建普通用户
 * Area: tests/e2e/views/users/User  (mirrors src/views/users)
 * Risk: high | Role: org-user
 *
 * Live e2e:
...
const MS_ID = 'MS-104214'
const TITLE = `用户管理-用户列表：使用系统管理员创建普通用户`
const STEPS = [
  {
...
const OUTCOMES = [
  "[1] 返回创建用户表单。",
  "[2] 创建系统用户成功。返回用户列表，可查看到该用户信息，且系统角色字段显示系统管理员。使用该用户登录，其拥有的权限：只有工作台"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：使用系统管理员创建普通用户`
