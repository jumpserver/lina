/**
 * Auto-generated from QA Mode B draft MS-104204
 * Source title: 用户管理-用户列表：使用系统管理员创建系统审计员
...
const MS_ID = 'MS-104204'
const TITLE = `用户管理-用户列表：使用系统管理员创建系统审计员`
const STEPS = [
  {
...
const OUTCOMES = [
  "[1] 返回创建用户表单。",
  "[2] 创建系统用户成功。返回用户列表，可查看到该用户信息，且系统角色字段显示系统管理员。使用该用户登录，其拥有的权限：包括审计台、工作台。且可切换到任意组织查看日志审计"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：使用系统管理员创建系统审计员`
