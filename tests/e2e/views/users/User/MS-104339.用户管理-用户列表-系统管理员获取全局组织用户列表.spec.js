/**
 * Auto-generated from QA Mode B draft MS-104339
 * Source title: 用户管理-用户列表：系统管理员获取全局组织用户列表
 * Area: tests/e2e/views/users/User  (mirrors src/views/users)
 * Risk: medium | Role: org-user
 *
 * Live e2e:
...
const MS_ID = 'MS-104339'
const TITLE = `用户管理-用户列表：系统管理员获取全局组织用户列表`
const STEPS = [
  {
    "action": "用户管理-用户列表：系统管理员获取全局组织用户列表",
    "id": "step-1",
    "sideEffect": "none"
  }
]
const OUTCOMES = [
  "[1] 返回全局组织用户列表信息，获取的是所有用户的信息。左上角有创建、更多操作按钮。右上角有搜索框、导出、刷新按钮。无邀请用户按钮，无导入按钮。且全局组织下，可创建用户，更新、删除用户。可批量禁用/激活删除用户。无法导入用户。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：系统管理员获取全局组织用户列表`
