/**
 * Auto-generated from QA Mode B draft MS-104410
 * Source title: 用户管理-用户列表：组织管理员获取其有权限的组织用户列表
...
const MS_ID = 'MS-104410'
const TITLE = `用户管理-用户列表：组织管理员获取其有权限的组织用户列表`
const STEPS = [
  {
    "action": "用户管理-用户列表：组织管理员获取其有权限的组织用户列表",
    "id": "step-1",
    "sideEffect": "none"
  }
]
const OUTCOMES = [
  "[1] 返回该组织用户列表信息，且数据是正确的。左上角有创建、邀请用户、更多操作按钮。右上角有搜索框、导入、导出、刷新按钮。默认一页显示15条数据，上翻、下翻数据显示正确。仅能获取其有权限的组织用户列表。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：组织管理员获取其有权限的组织用户列表`
