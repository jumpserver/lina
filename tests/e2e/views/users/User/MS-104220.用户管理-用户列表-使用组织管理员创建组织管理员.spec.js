/**
 * Auto-generated from QA Mode B draft MS-104220
 * Source title: 用户管理-用户列表：使用组织管理员创建组织管理员
...
const MS_ID = 'MS-104220'
const TITLE = `用户管理-用户列表：使用组织管理员创建组织管理员`
const STEPS = [
  {
  ...
  {
    "action": "在创建用户表单上：填写名称、用户名、邮箱，系统角色选系统管理员、组织角色选用户，其他按要求填写，提交",
    "id": "step-2",
    "sideEffect": "reversible"
  }
]
const OUTCOMES = [
  "[1] 返回创建用户表单。",
  "[2] 创建系统用户成功。返回用户列表，可查看到该用户信息，且系统角色字段显示系统管理员。使用该用户登录，其拥有权限：包括控制台、审计台、工作台、系统设置，可对资源进行增删改查。不可切换到任意组织，只能在其有权限的组织下。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：使用组织管理员创建组织管理员`
