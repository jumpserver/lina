/**
 * Auto-generated from QA Mode B draft MS-104280
 * Source title: 用户管理-用户列表：创建用户，设置用户有效期
...
const MS_ID = 'MS-104280'
const TITLE = `用户管理-用户列表：创建用户，设置用户有效期`
const STEPS = [
  {
    "action": "在创建用户表单上，设置用户有效期：为过期时间，其他按照要求填写，提交",
    "id": "step-1",
    "sideEffect": "reversible"
  },
  {
    "action": "在创建用户表单上，设置用户有效期：未过期，其他按照要求填写，提交",
    "id": "step-2",
    "sideEffect": "reversible"
  }
]
const OUTCOMES = [
  "[1] 创建用户成功，使用该用户登录，输入正确账号密码，登录失败。提示：此账号已过期。",
  "[2] 创建用户成功，使用该用户登录，入正确账号密码，登录成功。"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 用户管理-用户列表：创建用户，设置用户有效期`
