/**
 * Auto-generated from QA Mode B draft MS-100730
 * Source title: 测试token登录，查看用户列表是否有记录登录时间，防止被认为不活跃用户被禁用。
...
const MS_ID = 'MS-100730'
const TITLE = `测试token登录，查看用户列表是否有记录登录时间，防止被认为不活跃用户被禁用。`
const STEPS = [
  {
    "action": "测试token登录，查看用户列表是否有记录登录时间，防止被认为不活跃用户被禁用。",
    "id": "step-1",
    "sideEffect": "reversible"
  }
]
const OUTCOMES = [
  "[1] 用户列表记录该用户最后一次token登录时间。（token登录模拟，可参考备注）"
]
const BEHAVIOR = `模块: JumpServer / 控制台 / 用户管理 / 用户列表；验证: 测试token登录，查看用户列表是否有记录登录时间，防止被认为不活跃用户被禁用。`
