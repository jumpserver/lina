...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableConfig: {
        url: '/api/v1/accounts/change-secret-accounts/',
        columns: [
        ...
        columnsMeta: {
          ...
          execution_id: {
            ...
            formatterArgs: {
              drawer: true,
              can: true,
              getTitle: ({ row }) => row.execution_id,
              getDrawerTitle: ({ row }) => row.execution_id,
              getRoute: ({ row }) => ({
                name: 'AccountChangeSecretExecutionDetail',
                params: { id: row.execution_id }
              })
            }
          },
          status: {
            ...
            formatter: (row) => {
              ...
              if (statusMap[row.status]) {
                return <span>{ vm.$t(statusMap[row.status]) }</span>
              }
              return <span>–</span>
            }
          },
              ...
              extraActions: [
                {
                  ...
                  callback: ({ row }) => {
                    this.$axios.post(
                      '/api/v1/accounts/change-secret-accounts/',
