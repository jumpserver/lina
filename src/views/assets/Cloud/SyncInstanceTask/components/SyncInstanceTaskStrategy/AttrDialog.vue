<template>
  <Dialog
    ...
    v-on="$listeners"
  >
    <GenericCreateUpdateForm :fields="fields" :fields-meta="fieldsMeta" />
    <div>
      <AttrInput
...
export default {
  name: 'AttrDialog',
  components: { Dialog, AttrInput, GenericCreateUpdateForm },
  props: {
    form: {
      type: Object,
      default: () => ({})
  ...
  data() {
    ...
    return {
      fields: [
        ['条件配置', ['rule_name', 'rules']],
        ['动作配置', ['action_name', 'actions']]
      ],
      fieldsMeta: {
        rule_name: {
          rules: [Required],
